...
let RDWR_EXCL = fs.constants.O_CREAT | fs.constants.O_TRUNC | fs.constants.O_RDWR | fs.constants.O_EXCL;

// Windows refuses to delete read-only files (Git, for instance, marks pack files
// read-only), so `fs.rm` throws EPERM where the old rimraf-based cleanup cleared
// the read-only bit first. Recursively make the tree writable, then retry.
function makeWritableSync(targetPath) {
  let stats;
  try {
    stats = fs.lstatSync(targetPath);
  } catch (err) {
    return; // gone already — nothing to do
  }
  try {
    fs.chmodSync(targetPath, 0o700);
  } catch (err) {
    // best effort; the retried rm will surface any real failure
  }
  if (stats.isDirectory()) {
    ...
    for (let entry of entries) {
      makeWritableSync(path.join(targetPath, entry));
    }
  }
}
...
function forceRemoveSync(targetPath) {
  try {
    fs.rmSync(targetPath, { recursive: true, force: true, maxRetries: 6 });
  } catch (err) {
...
const mkdir = (affixes, callback) => {
  ...
  fs.mkdir(dirPath, { recursive: true, mode: 0o700 }, (err) => {
    if (!err) {
